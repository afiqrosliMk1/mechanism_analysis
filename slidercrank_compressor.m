% code save as from slidercrank_force_analysis.m
% force analysis of an air compressor
% Afiq Rosli, 3/5/2025
...
p = 0.016;  % distance from point C to CM of piston

% inertial properties
m2 = 1.551; % crank mass (kg)
m3 = 0.096; % con rod mass (kg)
m4 = 0.163; % piston mass (kg)
I2 = 0.001939; % crank moment of inertia (kg.m^2)
I3 = 0.000189; % con rod moment of inertia (kg.m^2) 
...
omega2 = 1200*2*pi/60; %1200rpm to rad/s 
omega3 = zeros(1, N);
ddot = zeros(1, N);
...
% pressure constants
h = 0.015; % head space (m)
dia = 0.050; % diameter of the piston (m)
A = pi*(dia/2)^2; % area of the piston

% friction constants
mu = 0.1; % coefficient of friction btw piston and cylinder
Ff = 0; % friction force starts at zero

% force analysis
Patm = 101e3; %atmostpheric pressure [kPa]
FP = zeros(2, N); % pressure force on piston [N]
P = zeros(1, N); % pressure acting on piston [Pa]
...
for i=1:N
    ...
    PF = dot(FP(:, i), v4(:, i));
    PT = T2(i)*omega2;
    Pfriction = dot(FD(:, i), v4(:, i)); 
    %friction power is subtracted bcs it is doing negative work on linkage
    PExt(i) = PF + PT - Pfriction;
...
%plot torque or cylinder pressure or friciton force
nexttile(6);
%plot(theta2*180/pi, T2);
%plot(theta2*180/pi, P*1e-6);
plot(theta2*180/pi, FD(1, :));
hold on
plot(theta2*180/pi, FD(2, :));
xlim([0 360]);
grid on
